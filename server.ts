...
function processVotingResults(roomId: string) {
  ...
  // Let's check who was voted out
  if (!votedOutId) {
    // Nobody got voted out (e.g. nobody voted) -> Spy wins!
    room.roundWinner = 'spies';
    awardPoints(roomId, 'spies');
    transitionToScoreboard(roomId);
  } else {
    // Player got voted out. Check if they are a spy
    const votedPlayer = room.players.find(p => p.id === votedOutId);
    if (votedPlayer?.role === 'spy') {
      // Spy was caught! Transition to SPY_GUESS phase so they can try to guess the secret word
      ...
      // Innocent civilian voted out -> Spy wins!
      room.roundWinner = 'spies';
      awardPoints(roomId, 'spies');
      transitionToScoreboard(roomId);
    }
...
function startNewRound(roomId: string) {
  ...
  room.settings.activePacks.forEach(packId => {
    const custom = customPacksMap.get(packId);
    if (custom) {
      selectedWords.push(...custom.words);
    } else {
  ...
  if (selectedWords.length === 0) {
    // Default to Classic if no packs active
    const classicPack = BUILT_IN_PACKS.find(p => p.id === 'classic');
    if (classicPack) selectedWords.push(...classicPack.words);
  }
