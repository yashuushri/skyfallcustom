...
import { initDatabase, saveRoom, loadRoom, logMatch, cleanStaleRooms, getIsDbConnected } from './db';

// Intercept console.log and console.error to capture server logs for the developer portal
...
app.get('/api/room/:roomId', async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      isLobbyLocked: !!room.settings.isLobbyLocked,
      settings: {
...
function checkAndScheduleRoomCleanup(roomId: string) {
  const room = rooms.get(roomId);
  if (!room) return;

  const connectedCount = room.players.filter(p => p.isConnected && !p.id.startsWith('P-bot')).length;
  if (connectedCount === 0) {
    if (!emptyRoomTimeouts.has(roomId)) {
      console.log(`[CLEANUP] Room ${roomId} has 0 connected players. Scheduling automatic deletion in 5 minutes...`);
      const timeout = setTimeout(() => {
        const r = rooms.get(roomId);
        if (r) {
          const checkConnected = r.players.filter(p => p.isConnected && !p.id.startsWith('P-bot')).length;
          if (checkConnected === 0) {
            if (r.timerInterval) clearInterval(r.timerInterval);
            rooms.delete(roomId);
            emptyRoomTimeouts.delete(roomId);
            console.log(`[CLEANUP] Room ${roomId} successfully deleted automatically after 5 minutes of being empty.`);
          }
        }
      }, 5 * 60 * 1000); // 5 minutes
    ...
    if (emptyRoomTimeouts.has(roomId)) {
      console.log(`[CLEANUP] Player connected/reconnected to room ${roomId}. Cancelling automatic 5-minute deletion.`);
      clearTimeout(emptyRoomTimeouts.get(roomId));
      emptyRoomTimeouts.delete(roomId);
    }
  }
}
...
function handleDisconnectGrace(roomId: string, playerId: string) {
  if (playerId.startsWith('P-bot')) return;
  const room = rooms.get(roomId);
  if (!room) return;
  ...
  // Host migration immediately upon disconnect
  if (player.isHost) {
    const nextHost = room.players.find(p => p.isConnected && !p.id.startsWith('P-bot') && p.id !== playerId);
    if (nextHost) {
      player.isHost = false;
      nextHost.isHost = true;
      console.log(`Immediate Host migrated to ${nextHost.name} in room ${roomId} because original host disconnected.`);
      io.to(roomId).emit('notification', { message: `Host left. ${nextHost.name} is now the Host.` });
    }
  }
  ...
  // If all players are now offline, schedule automatic deletion in 5 mins
  const connectedHumanCount = room.players.filter(p => p.isConnected && !p.id.startsWith('P-bot')).length;
  ...
  const timeout = setTimeout(() => {
    ...
    // If lobby is empty, check and schedule room cleanup
    if (r.players.filter(p => p.isConnected && !p.id.startsWith('P-bot')).length === 0) {
      checkAndScheduleRoomCleanup(roomId);
      return;
    }

    // Host migration backup (if not already migrated)
    if (player.isHost) {
      const nextHost = r.players.find(p => p.isConnected && !p.id.startsWith('P-bot'));
      if (nextHost) {
        nextHost.isHost = true;
        console.log(`Backup Host migrated to ${nextHost.name} in room ${roomId}.`);
        io.to(roomId).emit('notification', { message: `Host left. ${nextHost.name} is now the Host.` });
      }
    }
    ...
    if (r.phase !== 'LOBBY' && r.phase !== 'END_GAME') {
      ...
      if (activePlayers.length < 1 || activeSpies.length === 0) {
        ...
        r.players.forEach(p => {
          p.isReady = p.id.startsWith('P-bot');
          p.role = undefined;
          p.votedFor = null;
        });
        r.actualSpies = [];
    ...
    broadcastState(roomId);
  }, 180000); // 180 second (3 minute) grace period to reconnect
...
function checkAndAdvanceBotTurn(roomId: string) {
  ...
  if (currentSpeakerId && currentSpeakerId.startsWith('P-bot')) {
    setTimeout(() => {
      const r = rooms.get(roomId);
      if (!r || r.phase !== 'DISCUSSION') return;
      if (r.questionOrder[r.currentQuestionIndex] === currentSpeakerId) {
...
function getSanitizedStateForPlayer(roomId: string, playerId: string) {
  ...
  const sanitizedPlayers = filteredPlayers.map(p => {
    ...
    // Make actual votes visible in real-time during VOTING/SPY_GUESS as requested
    let displayVote: string | null = p.votedFor || null;
...
function transitionToVoting(roomId: string) {
  ...
  // If bots are in room, let them vote after a short delay!
  const botsInRoom = room.players.filter(p => p.id.startsWith('P-bot') && p.role && p.role !== 'spectator');
  if (botsInRoom.length > 0) {
    setTimeout(() => {
      const r = rooms.get(roomId);
      if (!r || r.phase !== 'VOTING') return;
      
      const bots = r.players.filter(p => p.id.startsWith('P-bot') && p.role && p.role !== 'spectator');
      bots.forEach(bot => {
        if (bot.votedFor === null) {
          // Vote for a random other active player who is connected
          const candidates = r.players.filter(p => p.id !== bot.id && p.role && p.role !== 'spectator' && p.isConnected);
      ...
      if (allVoted) {
        broadcastState(roomId);
        setTimeout(() => {
          const roomToProcess = rooms.get(roomId);
          if (roomToProcess && roomToProcess.phase === 'VOTING') {
            const stillAllVoted = roomToProcess.players
              .filter(p => p.role && p.role !== 'spectator' && p.isConnected)
...
function processVotingResults(roomId: string) {
  ...
  const spyBots = room.players.filter(p => p.id.startsWith('P-bot') && p.role === 'spy');
  if (spyBots.length > 0) {
    spyBots.forEach((spyBot, index) => {
      setTimeout(() => {
        const r = rooms.get(roomId);
...
function startNewRound(roomId: string) {
  ...
  // Reset scores and match stats if starting a brand new game/match from the Lobby phase
  if (room.phase === 'LOBBY') {
    room.players.forEach(p => {
      p.score = 0;
      p.wins = 0;
  ...
  // Auto-ready all bots for reveal phase
  room.players.forEach(p => {
    if (p.id.startsWith('P-bot')) {
      p.isReady = true;
      room.readySet.add(p.id);
    }
  });
...
function isAuthorizedHost(room: any, playerId: string): boolean {
  ...
  if (actualHost && actualHost.id.startsWith('P-bot') && !playerId.startsWith('P-bot')) {
    return true;
  }
  return false;
}
...
io.on('connection', (socket: Socket) => {
  ...
  socket.on('join_room', async ({ roomId, name, playerId }: { roomId: string; name: string; playerId?: string }) => {
    ...
    if (existingPlayer) {
      ...
      const isBotTheHost = room.players.some(p => p.id.startsWith('P-bot') && p.isHost);
      if (!isBotTheHost) {
        const hasConnectedHumanHost = room.players.some(p => p.isHost && p.isConnected && !p.id.startsWith('P-bot') && p.id !== existingPlayer?.id);
      ...
      checkAndScheduleRoomCleanup(roomCode);
      return;
    }

    // New Player joining
    // Check if room is locked
    if (!isDevPlayer && room.settings.isLobbyLocked) {
      socket.emit('error', { message: 'This game lobby is currently locked by the host.' });
      return;
    }
    ...
    if (!finalName) {
      finalName = `Player ${room.players.filter(p => !p.invisible).length + 1}`;
    }
    let checkName = finalName;
    let counter = 1;
    while (room.players.some(p => p.name.toLowerCase().trim() === checkName.toLowerCase().trim())) {
      counter++;
    ...
    const newPlayer: Player = {
      id: newPlayerId,
      name: isDevPlayer ? 'lPu9c&]4a8$6' : finalName,
      isHost: false,
      isReady: false,
      score: 0,
      wins: 0,
    ...
    const isBotTheHost = room.players.some(p => p.id.startsWith('P-bot') && p.isHost);
    if (!isBotTheHost) {
      const hasConnectedHumanHost = room.players.some(p => p.isHost && p.isConnected && !p.id.startsWith('P-bot'));
      if (!hasConnectedHumanHost) {
        room.players.forEach(p => (p.isHost = false));
  ...
  socket.on('request_sync', async ({ roomId, playerId }: { roomId: string; playerId: string }) => {
    ...
    if (player) {
      ...
      const hasConnectedHumanHost = room.players.some(p => p.isHost && p.isConnected && !p.id.startsWith('P-bot') && p.id !== player.id);
      if (!hasConnectedHumanHost) {
        room.players.forEach(p => (p.isHost = false));
        player.isHost = true;
      }
  ...
  // Handle Add Bot (Host only)
  socket.on('add_bot', ({ roomId, playerId }: { roomId: string; playerId: string }) => {
    const roomCode = roomId.toUpperCase().trim();
    const room = rooms.get(roomCode);
    if (!room) return;
    ...
    if (room.players.length >= room.settings.maxPlayers) {
      socket.emit('error', { message: 'Lobby is full. Cannot add more bots.' });
      return;
    }

    // List of cool agent bot names
    const botNames = [
      '🤖 Agent Echo',
      '🤖 Agent Alpha',
      '🤖 Agent Bravo',
    ...
    if (!chosenName) {
      let counter = 1;
      while (room.players.some(p => p.name === `🤖 Agent Bot ${counter}`)) {
        counter++;
      }
      chosenName = `🤖 Agent Bot ${counter}`;
    }

    const botId = 'P-bot-' + Math.random().toString(36).substr(2, 9);
    const newBot: Player = {
      id: botId,
      name: chosenName,
      isHost: false,
    ...
    console.log(`Bot ${newBot.name} (${botId}) added to room ${roomCode}`);
    io.to(roomCode).emit('trigger_sfx', { type: 'join' });
    broadcastState(roomCode);
  });
  ...
  socket.on('toggle_spectator', ({ roomId, playerId, targetPlayerId }: { roomId: string; playerId: string; targetPlayerId?: string }) => {
    const roomCode = roomId.toUpperCase().trim();
    const room = rooms.get(roomCode);
    if (!room) return;

    if (room.settings.isLobbyLocked) {
      socket.emit('error', { message: 'Lobby is locked. Switching mode is not allowed.' });
      return;
    }
    ...
    let targetPlayer = caller;
    if (targetPlayerId) {
      // Only host can toggle another player's role
      if (!caller.isHost) {
        socket.emit('error', { message: 'Only the host can toggle another player\'s role.' });
        return;
      }
      const found = room.players.find(p => p.id === targetPlayerId);
      if (found) {
        targetPlayer = found;
      }
    }

    if (targetPlayer.role === 'spectator') {
      if (room.phase !== 'LOBBY' && room.phase !== 'END_GAME') {
        socket.emit('error', { message: 'Cannot join active game. Please wait for the round to end.' });
        return;
      }
      targetPlayer.role = undefined;
      targetPlayer.isReady = false;
      console.log(`${targetPlayer.name} switched to active player in ${roomCode}`);
    } else {
      targetPlayer.role = 'spectator';
      targetPlayer.isReady = false;
      console.log(`${targetPlayer.name} switched to spectator in ${roomCode}`);
    }
  ...
  socket.on('dev_change_phase', ({ roomId, playerId, phase }: { roomId: string; playerId: string; phase: GamePhase }) => {
    ...
    if (phase === 'DISCUSSION') {
      room.players.forEach(p => (p.isReady = p.id.startsWith('P-bot')));
      room.readySet.clear();
      startDiscussionTimer(roomCode);
      checkAndAdvanceBotTurn(roomCode);
  ...
  socket.on('player_diagnostics_update', ({ roomId, playerId, diagnostics }: { roomId: string; playerId: string; diagnostics: any }) => {
    const roomCode = roomId.toUpperCase().trim();
    const room = rooms.get(roomCode);
    if (!room) return;
  ...
  socket.on('dev_request_server_metrics', () => {
    ...
    rooms.forEach((room, rId) => {
      ...
      totalPlayers += pCount;
      const bCount = room.players.filter(p => p.id.startsWith('P-bot')).length;
      botCount += bCount;
      const sCount = room.players.filter(p => p.role === 'spectator').length;
      spectatorCount += sCount;
      if (room.phase !== 'LOBBY') {
        activeGamesCount++;
      }
      ...
      activeRoomsList.push({
        roomId: rId,
        roomName: room.settings.roomName || `Room ${rId}`,
        phase: room.phase,
    ...
    const metrics = {
      ...
      activeRoomsList,
      serverLogs,
      databaseType: isDbConnected ? 'Supabase Relational PostgreSQL Pool' : 'In-Memory Fallback Map',
      databaseStatus: isDbConnected ? 'HEALTHY' : 'FALLBACK_MEMORY',
      apiEndpoints: [
        { path: '/api/health', method: 'GET', status: 'ACTIVE' },
        { path: 'Socket.IO Namespace (/)', method: 'WS', status: 'ACTIVE' }
      ],
      healthServices: {
        renderBackend: {
          name: "Render Backend",
          provider: "Render.com Cloud Run Container",
          status: "ACTIVE",
          region: "Singapore (sin-1)",
          ingressPort: 3000,
          pingMs: 45
        },
        supabaseDatabase: {
          name: "Supabase Database",
          provider: "Supabase Managed Postgres (AP-Southeast)",
          status: isDbConnected ? "HEALTHY" : "FALLBACK_MEMORY",
          isConfigured: !!process.env.DATABASE_URL,
          poolSize: 20
        },
        vercelFrontend: {
          name: "Vercel Frontend",
          provider: "Vercel Edge Network / CDN",
          status: "ACTIVE",
          deployment: "spyfall-custom.vercel.app",
          protocol: "HTTP/3 (QUIC)"
        },
        socketIo: {
          name: "Socket.io Engine",
          status: "ACTIVE",
          activeSockets: totalPlayers,
          transport: "WebSocket + Long Polling"
        }
      }
    };

    socket.emit('dev_server_metrics_response', metrics);
  });

  // Handle Ping/Latency checks
  ...
  socket.on('emoji_reaction', ({ roomId, emoji }: { roomId: string; emoji: string }) => {
    const roomCode = roomId.toUpperCase().trim();
    io.to(roomCode).emit('emoji_reaction_received', {
