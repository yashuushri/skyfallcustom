...
const ANALYTICS_STORE_NAME = 'player_analytics';
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(ANALYTICS_STORE_NAME)) {
        db.createObjectStore(ANALYTICS_STORE_NAME, { keyPath: 'sessionId' });
      }
    };
...
export async function savePlayerAnalytics(analytics: PlayerAnalytics): Promise<void> {
  try {
    const db = await initIndexedDB();
    await new Promise<void>((resolve, reject) => {
    ...
    console.error('IndexedDB error saving player analytics:', error);
  }
}

export async function getAllPlayerAnalytics(): Promise<PlayerAnalytics[]> {
  try {
    const db = await initIndexedDB();
    return new Promise((resolve, reject) => {
    ...
    console.error('IndexedDB error getting player analytics:', error);
    return [];
  }
}

export async function deletePlayerAnalytics(sessionId: string): Promise<void> {
  try {
    const db = await initIndexedDB();
    await new Promise<void>((resolve, reject) => {
      const transaction = db.transaction(ANALYTICS_STORE_NAME, 'readwrite');
    ...
    console.error('IndexedDB error deleting player analytics:', error);
  }
}

export async function clearAllPlayerAnalytics(): Promise<void> {
  try {
    const db = await initIndexedDB();
    await new Promise<void>((resolve, reject) => {
      const transaction = db.transaction(ANALYTICS_STORE_NAME, 'readwrite');
    ...
    console.error('IndexedDB error clearing player analytics:', error);
  }
}
