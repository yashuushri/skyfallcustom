...
export default function WordPackManager({ 
  ...
  return (
    <div className="bg-[#1e1e1e] border border-[#2e2e2e] p-5 rounded-2xl space-y-4">
      ...
      {/* Elegant Toast Alert */}
      {/* Multiple Word Packs Toggle */}
      <div className="flex items-center justify-between bg-[#151515] p-3 rounded-xl border border-[#252525]">
        <div className="space-y-0.5 pr-2">
          <p className="text-xs font-bold text-neutral-200">Multiple Word Packs</p>
        </div>
        <button
          type="button"
          disabled={!isHost}
          onClick={() => {
            if (!isHost) return;
            soundManager.playClick();
            const nextVal = !multiplePacksEnabled;
            // If turning OFF and currently has multiple active, reduce to just the first one and alert
            if (!nextVal && activePacks.length > 1) {
              const keepPack = activePacks[0];
              onUpdatePacks([keepPack], customPacks);
              onErrorAlert("Multiple Word Packs disabled. Reverted selection to a single pack.");
            }
            onToggleMultiplePacks(nextVal);
          }}
          className={`relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
            multiplePacksEnabled ? 'bg-blue-600' : 'bg-neutral-800'
          } ${!isHost ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
          <span
            className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
              multiplePacksEnabled ? 'translate-x-5' : 'translate-x-0'
            }`}
          />
        </button>
      </div>
