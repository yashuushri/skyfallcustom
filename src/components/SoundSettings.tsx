...
import { Volume2, VolumeX, User, Globe, Check, Save, LogOut, Eye, EyeOff, Radio } from 'lucide-react';
...
export default function SoundSettings({
  ...
  const [ambient, setAmbient] = useState<boolean>(() => {
    return soundManager.getIsAmbientPlaying();
  });
  ...
  useEffect(() => {
    const savedAmbient = localStorage.getItem('sfxAmbient');
    if (savedAmbient === 'true' && !soundManager.getIsAmbientPlaying() && !muted) {
  ...
  const handleToggleAmbient = () => {
    ...
    const nextAmbient = !ambient;
    setAmbient(nextAmbient);
    localStorage.setItem('sfxAmbient', nextAmbient.toString());
    if (nextAmbient) {
      soundManager.playAmbientHum();
    } else {
    ...
    <div className="bg-[#1e1e1e] border border-[#2e2e2e] p-6 rounded-2xl max-w-sm w-full space-y-6 shadow-2xl">
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-3 pt-2 border-t border-[#2e2e2e]">
          ...
          {/* Spaceship Ambient Hum Toggle */}
          <div className="flex items-center justify-between bg-[#151515] p-3 rounded-xl border border-[#262626] mt-1">
            <div className="flex flex-col text-left">
              <span className="text-xs font-bold text-neutral-400 uppercase tracking-wider flex items-center gap-1.5">
                <Radio className={`h-3.5 w-3.5 ${ambient && !muted ? 'text-emerald-400 animate-pulse' : 'text-neutral-500'}`} />
                Ambient Drone
              </span>
              <span className="text-[9px] text-neutral-500 leading-tight mt-0.5">
                Spaceship engine hum & ventilation
              </span>
            </div>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[10px] font-mono font-bold transition-all border cursor-pointer ${
                muted
                  ...
                  : ambient 
                    ...
                    : 'bg-[#2a2a2a] text-neutral-400 border-[#3e3e3e] hover:bg-[#353535]'
              }`}
            >
              {ambient && !muted ? 'ON' : 'OFF'}
            </button>
          </div>
        </div>
