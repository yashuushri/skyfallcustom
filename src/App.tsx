            ...
            >
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  onClick={() => {
                    soundManager.playClick();
                    if (window.confirm("Are you sure you want to restart the match? This returns all players to the lobby. Player scores will be kept.")) {
        ...
        {gameState && (
          <div className="space-y-6">
            ...
            {gameState.phase === 'LOBBY' && (
              <div className="bg-[#1e1e1e] border border-[#2e2e2e] rounded-2xl p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                  ...
                  {/* Button 2: Copy Invite Link OR Share button */}
                  {navigator.share ? (
                    <button
                      onClick={handleShareRoom}
                      className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-[#2a2a2a] border border-[#3e3e3e] hover:bg-[#353535] text-neutral-200 hover:text-white text-xs font-bold transition-colors cursor-pointer"
                ...
                {/* If navigator.share exists, show a secondary Copy Invite Link button just in case */}
                {navigator.share && (
                  <div className="flex justify-center">
                    <button
                      onClick={handleCopyInviteLink}
                      className="text-[10px] font-mono font-bold text-neutral-500 hover:text-neutral-300 flex items-center gap-1 cursor-pointer bg-transparent border-none"
                    >
                      {successMessage === 'Invite Link Copied' ? 'Invite Link Copied!' : 'Copy Invite Link'}
                    </button>
                  </div>
                )}
              </div>
                              ...
                              <div 
                                ...
                                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-semibold ${
                                  isMe 
                                    ...
                                    : 'bg-[#151515] border-[#252525] text-neutral-400'
                                }`}
                              >
                                <div className="h-1.5 w-1.5 rounded-full bg-neutral-500" />
                                <span>{p.name}</span>
                                {isMe && <span className="text-[8px] font-black uppercase text-blue-400 bg-blue-500/10 px-1 rounded ml-1">Me</span>}
                              </div>
                            );
                          })}
                        </div>
                      ...
                      .map(p => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full py-3.5 px-6 rounded-xl border text-sm font-bold tracking-wide transition-all ${
                              isMe
                                ? 'bg-[#151515] border-neutral-900 text-neutral-600 cursor-not-allowed opacity-40'
                                : hasVotedForThem
                                  ? 'bg-[#303030] border-[#505050] text-white shadow-md'
            ...
            {gameState.phase === 'SPY_GUESS' && (() => {
              ...
              const isSpyPlayer = currentPlayer?.role === 'spy' && gameState.votedOutPlayerId === playerId;
                  ...
                  {isSpyPlayer ? (
                    ...
                    <div className="p-6 bg-[#151515] border border-[#252525] rounded-xl space-y-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-[11px] text-neutral-500 leading-relaxed">Please remain silent while the spy tries to recall the options.</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {gameState.phase === 'SCOREBOARD' && (() => {
              ...
              return (
                <div className="max-w-md w-full mx-auto bg-[#1e1e1e] border border-[#2e2e2e] rounded-2xl p-6 space-y-6 text-center select-none">
                  ...
                  <div className="space-y-1">
                    {didIWin ? (
                      <>
                        ...
                        <p className="text-xs text-neutral-400 leading-relaxed max-w-xs mx-auto">
                          {isSpy 
                            ? "Excellent infiltration! They couldn't reveal you." 
                            : "Splendid work! The spy has been neutralized."}
                        </p>
                      </>
                    ) : (
                      <>
                        ...
                        <p className="text-xs text-neutral-400 leading-relaxed max-w-xs mx-auto">
                          {isSpy
                            ? "Unfortunate! The civilians discovered your identity."
                            : `The spy was ${spyPlayer ? spyPlayer.name : 'unknown'}`}
                        </p>
                      </>
                    )}
                  </div>
                  ...
                  <div className="bg-[#151515] border border-[#252525] p-4 rounded-xl space-y-1">
                    <p className="text-[10px] font-mono font-bold tracking-widest text-neutral-500 uppercase">
                      Secret Location
                    </p>
                    <p className="text-2xl font-black text-[#3b82f6] uppercase tracking-wider">
                      {gameState.secretWord}
                    </p>
                  </div>
